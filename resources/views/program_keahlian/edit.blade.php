    ...
    <form action="{{ route('program_keahlian.update', $programKeahlian->id_program_keahlian) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <select class="form-control" id="id_bidang_keahlian" name="id_bidang_keahlian">
                ...
                <option value="{{ $bidangKeahlian->id_bidang_keahlian }}" {{  $programKeahlian->id_bidang_keahlian == $bidangKeahlian->id_bidang_keahlian ? 'selected' : '' }}>{{ $bidangKeahlian->kode_bidang_keahlian }}</option>
                @endforeach
            </select>
